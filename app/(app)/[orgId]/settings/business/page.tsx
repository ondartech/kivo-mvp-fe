...
export default function BusinessIdentityPage() {
  ...
  const pendingVerification = useMemo(() => verifications.find((v) => v.status === "PENDING_CONFIRMATION") ?? null, [verifications]);
  ...
  return (
    <div className="space-y-6 max-w-[880px]">
      <PageHeader title="Business Identity" description="Business identity used in invoices — name, address, logo. Frozen in snapshots after issue. Verification builds buyer trust on invoices and public views." />
      <Card>
        ...
        <CardContent className="space-y-3">
          <fieldset className="flex flex-col sm:flex-row gap-3" aria-label="Business structure">
            <label className="flex-1 flex items-center gap-2 rounded-lg border p-3 cursor-pointer has-[:checked]:border-brand has-[:checked]:bg-brand/5">
              <input type="radio" name="business_structure" value="UNREGISTERED" checked={structureDraft === "UNREGISTERED"} onChange={() => setStructureDraft("UNREGISTERED")} className="h-4 w-4" aria-label="Unregistered business" />
              <span className="text-sm font-medium">Unregistered</span>
            </label>
            <label className="flex-1 flex items-center gap-2 rounded-lg border p-3 cursor-pointer has-[:checked]:border-brand has-[:checked]:bg-brand/5">
              <input type="radio" name="business_structure" value="REGISTERED" checked={structureDraft === "REGISTERED"} onChange={() => setStructureDraft("REGISTERED")} className="h-4 w-4" aria-label="Registered business" />
              <span className="text-sm font-medium">Registered (CAC)</span>
            </label>
          </fieldset>
          <p className="text-xs text-muted-foreground">You can change this later. Switching to Unregistered marks verification as not applicable; switching back keeps your last verification but you should re-verify if your legal name changes.</p>
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
          ...
          <form className="space-y-4" onSubmit={profileForm.handleSubmit(onSaveProfile)} aria-label="Business profile form">
            <div>
              ...
              {profileForm.formState.errors.legal_name ? (
                ...
                <p id="legal_name-help" className="mt-1 text-xs text-muted-foreground">Displayed on invoices and public pages. Will be frozen into the invoice snapshot when you issue.</p>
              )}
            </div>
      ...
      {isRegisteredStructure ? (
        <Card>
          ...
          <CardContent className="space-y-4">
            <form className="space-y-3" onSubmit={cacForm.handleSubmit(onLookup)} aria-label="CAC lookup form">
              <div className="grid sm:grid-cols-[160px_1fr_auto] gap-3 items-end">
                <div>
                  ...
                  <select id="identifier_type" className="mt-1 h-9 w-full rounded-md border border-input bg-surface px-2 text-sm" aria-label="Identifier type" {...cacForm.register("identifier_type")}>
                ...
                <div>
                  ...
                  {cacForm.formState.errors.identifier ? (
                    ...
                    <p id="identifier-help" className="mt-1 text-xs text-muted-foreground">CAC lookup by RC/BN. The name comes back from CAC — you don’t type it.</p>
                  )}
                </div>
            ...
            {lookupResult ? (
              <div className={`rounded-lg border p-4 space-y-3 ${lookupResult.found ? "bg-success-subtle border-success/20" : "bg-neutral-50 border-dashed"}`} aria-live="polite">
                ...
                {lookupResult.found ? (
                  <>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button variant="secondary" size="sm" disabled={!canMutate || isExpired} onClick={() => onUseRegisteredName(lookupResult.registered_name)}>
                        Use registered name
                      </Button>
                      <Button size="sm" loading={confirmingId === lookupResult.verification_id} disabled={!canMutate || isExpired || confirmingId !== null} onClick={() => onConfirm(lookupResult.verification_id)} aria-label="Confirm verification">
                  ...
                  <>
                    <p className="text-sm">No matching {lookupResult.identifier_type} {lookupResult.identifier} in CAC. Check the number for typos or continue with manual entry — invoices remain usable while unverified.</p>
